...
# Written by Ross Girshick, Xinlei Chen, Lichao Wang
# --------------------------------------------------------
...
class imdb(object):
    ...
    def _get_cache(self):
        return osp.abspath(osp.join(cfg.DATA_DIR, 'cache'))
    ...
    def get_file(self, index):
        image_file = self.image_path_at(index)
        ann_file = os.path.splitext(image_file)[0] + ".xml"
        return image_file, ann_file

    def get_img_info(self, index):
        _, annotation_file = self.get_file(index)
    ...
    def _get_widths(self):
        return [
            self.get_img_info(i)['width']
            for i in tqdm(range(self.num_images))
        ]

    def _get_heights(self):
        return [
            self.get_img_info(i)['height']
            for i in tqdm(range(self.num_images))
        ]
